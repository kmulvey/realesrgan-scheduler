...
func runWorkers(cmdPath, outputPath string, numGPUs int, originalImages chan path.WatchEvent, upsizedImages chan path.Entry) {
	...
	for i := 0; i <= numGPUs; i++ {
		...
		errorChans[i] = errors
		go upsizeWorker(cmdPath, outputPath, i, originalImages, upsizedImages, errors)
	}
